...
def crawl_naver_movie(number):
    ...
    sql = "INSERT INTO director_prize (director_id, movie_id, prize, year, award_result, award_info, image) VALUES(%s, %s, %s, %s, %s, %s, %s);"   
                    ...
                    try:
                        ...
                        if prizelist:
                            ...
                            for tr in prizelist:
                                ...
                                try:
                                    year = tr.select_one('td.tbl_sub1 > strong').text
                                    # print(year)
                                except:
                                    print(year)
                                
                                prize = tr.select_one('td.tbl_sub2 > a')
                                ...
                                # prize 이미지
                                image = tr.select_one('td.tbl_sub1 > a > img')['src']
                                
                                # print(prize.text)
                                prizelist2 = tr.select('td.tbl_sub3 > div')
                                
                                for div in prizelist2:
                                    award_result = div.select_one('span.award_result').text.replace("\n", "").replace("\t", "")
                                    award_info = div.select_one('span.award_info').text.replace("\n", "").replace("\t", "")
                                    # print(award_result)
                                    # print(award_info)      
                                    
                                    DataList = (director_id, movie_id, prize.text, year, award_result, award_info, image)
